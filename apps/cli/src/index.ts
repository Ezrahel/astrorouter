...
async function handleSwitch(
  ...
  if (config.authType !== "user" || !config.userToken) {
    console.log(chalk.red("❌ Please login first with: astroroute login"));
    process.exit(1);
  }
...
async function handleWhoami(
  ...
  try {
    ...
    if (orgs.length > 1) {
      console.log(
        chalk.dim(
          `\nYou have access to ${orgs.length} organizations. Use 'astroroute switch' to change.`,
        ),
      );
    }
  } catch (error) {
...
async function handleStartFromConfig(
  ...
  const defaultConfigPath = path.join(process.cwd(), "astroroute", "config.toml");
  ...
  if (!authConfig) {
    console.log(chalk.red("Not logged in. Run: astroroute login"));
    process.exit(1);
  }
...
function printHelp() {
  console.log(chalk.cyan("\nUsage:"));
  console.log(chalk.cyan(" astro-route login           Login via browser"));
  console.log(
    chalk.cyan("  astroroute start           Start tunnels from config.toml"),
  );
  console.log(chalk.cyan("  astroroute validate-config Validate config.toml file"));
  console.log(chalk.cyan("  astroroute <port>          Start HTTP tunnel"));
  console.log(chalk.cyan("  astroroute http <port>     Start HTTP tunnel"));
  console.log(chalk.cyan("  astroroute tcp <port>      Start TCP tunnel"));
  console.log(chalk.cyan("  astroroute udp <port>      Start UDP tunnel"));
  console.log(chalk.cyan("  astroroute switch [org]    Switch organization"));
  console.log(chalk.cyan("  astroroute whoami          Show current user"));
  console.log(chalk.cyan("  astroroute logout          Logout"));
  console.log(chalk.cyan("  astroroute version         Show version"));
  console.log(chalk.cyan("  astroroute help            Show this help message"));
  console.log(chalk.cyan("\nOptions:"));
  console.log(
    chalk.cyan(
...
async function main() {
  ...
  const serverUrl =
    ...
    (isDev ? "ws://localhost:3547" : "wss://api.astroroute.dev/");
  const webUrl =
    process.env.OUTRAY_WEB_URL ||
    (isDev ? "http://localhost:3000" : "https://astroroute.dev");

  const configManager = new ConfigManager(isDev);

  if (command === "version" || command === "-v" || command === "--version") {
    console.log(`astroroute version ${version}`);
    return;
  }
  ...
  if (command === "http") {
    ...
    if (!portArg) {
      ...
      console.log(chalk.cyan("Usage: astroroute http <port>"));
      process.exit(1);
    }
    ...
    if (!portArg) {
      console.log(chalk.red("❌ Please specify a port"));
      console.log(chalk.cyan("Usage: astroroute tcp <port>"));
      process.exit(1);
    }
    localPort = parseInt(portArg, 10);
    remainingArgs = args.slice(2);
    ...
    if (!portArg) {
      console.log(chalk.red("❌ Please specify a port"));
      console.log(chalk.cyan("Usage: astroroute udp <port>"));
      process.exit(1);
    }
    localPort = parseInt(portArg, 10);
    remainingArgs = args.slice(2);
  ...
  if (keyValue) {
    ...
    try {
      ...
      console.log(chalk.cyan("Try running: astroroute login"));
      process.exit(1);
    }
  }
