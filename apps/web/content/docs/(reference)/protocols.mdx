...
description: Open non-HTTP services over TCP or UDP with the AstroRoute CLI.
icon: TbBuildingTunnel
---

AstroRoute can tunnel raw TCP and UDP traffic so you can share things like SSH, databases, syslog, or game servers.

## TCP: expose any TCP service (SSH, DB, etc.)

```bash
astroroute tcp 22 --remote-port 2222
```
...
```bash
astroroute udp 514 --remote-port 1514
```
...
- **Port already in use**: retry without `--remote-port` to let AstroRoute pick one, or choose another.
