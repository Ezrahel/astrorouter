...
<Callout type="info">
Password-protected tunnels are available on **paid plans** (Ray, Beam, and Pulse). [View pricing →](https://astroroute.dev/pricing)
</Callout>
...
```bash
astroroute 3000 --password mysecretpassword
```
...
You can also set a password in your `astroroute/config.toml`:
...
```bash
curl -u :mysecretpassword https://my-tunnel.tunnel.astroroute.app/api/health
```
...
curl -H "Authorization: Basic $(echo -n ':mysecretpassword' | base64)" \
  https://my-tunnel.tunnel.astroroute.app/api/health
```
