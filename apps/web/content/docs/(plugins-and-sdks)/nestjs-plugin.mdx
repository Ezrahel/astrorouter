---
title: NestJS Plugin
description: Automatically expose your NestJS dev server via AstroRoute tunnel
icon: SiNestjs
---

The `@astroroute/nest` plugin automatically creates an AstroRoute tunnel when your NestJS application starts, giving you a public URL to share your local development environment.
...
import { NestFactory } from '@nestjs/core';
import { AppModule } from './app.module';
import { astroroute, AstroroutePluginOptions } from '@astroroute/nest';

async function bootstrap() {
  const app = await NestFactory.create(AppModule);
  await app.listen(3000);

  const options: AstroroutePluginOptions = {
    subdomain: 'my-api',
    apiKey: process.env.OUTRAY_API_KEY,
...
2. Establishes a WebSocket tunnel to AstroRoute servers
...
- Check that you've called `await app.listen()` before `await astroroute(app)`
- Verify your server is listening on a TCP port

### Port detection issues

If the plugin can't detect the port automatically:
- Make sure you `await app.listen(port)` before calling `astroroute(app)`
- Or pass the port explicitly: `astroroute(app, { port: 3000 })`
