...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (path === '/' && method === 'GET') {
      ...
      return res.end(JSON.stringify({ message: 'Welcome to test server', timestamp: Date.now() }));
    }
    ...
    if (path === '/users' && method === 'POST') {
      ...
      return res.end(JSON.stringify({ id: 4, ...data, createdAt: new Date().toISOString() }));
    }

    if (path.match(/^\/users\/\d+$/) && method === 'GET') {
      const id = path.split('/')[2];
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ id: parseInt(id), name: 'User ' + id, email: `user${id}@example.com` }));
    }
    ...
    if (path.match(/^\/users\/\d+$/) && method === 'PUT') {
      ...
      const data = body ? JSON.parse(body) : {};
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ id: parseInt(id), ...data, updatedAt: new Date().toISOString() }));
    }
    ...
    if (path === '/posts' && method === 'GET') {
      ...
      return res.end(JSON.stringify({
        ...
        pagination: { page: parseInt(page), limit: parseInt(limit), total: 100 }
      }));
    }

    if (path === '/upload' && method === 'POST') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ success: true, size: body.length, message: 'File received' }));
    }

    if (path === '/echo' && method === 'POST') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ echo: body ? JSON.parse(body) : null, headers: req.headers }));
    }
    ...
    if (path === '/error/400') {
      ...
      return res.end(JSON.stringify({ error: 'Bad Request', message: 'Invalid input' }));
    }
    ...
    if (path === '/error/403') {
      ...
      return res.end(JSON.stringify({ error: 'Forbidden', message: 'Access denied' }));
    }

    if (path === '/error/404') {
      res.writeHead(404, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Not Found', message: 'Resource not found' }));
    }

    if (path === '/error/500') {
      res.writeHead(500, { 'Content-Type': 'application/json' });
      return res.end(JSON.stringify({ error: 'Internal Server Error', message: 'Something went wrong' }));
    }

    if (path === '/html' && method === 'GET') {
      res.writeHead(200, { 'Content-Type': 'text/html' });
      return res.end('<html><body><h1>Hello HTML</h1><p>This is HTML content</p></body></html>');
    }
