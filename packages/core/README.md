...
```bash
npm install @astroroute/core
# or
pnpm add @astroroute/core
# or
yarn add @astroroute/core
```

## Usage

```ts
import { OutrayClient } from '@astroroute/core';
...
```ts
import { encodeMessage, decodeMessage } from '@astroroute/core';
...
```ts
import { ErrorCodes } from '@astroroute/core';
...
If you're building a framework plugin, use `@astroroute/core` as the foundation:

```ts
import { OutrayClient, type OutrayClientOptions } from '@astroroute/core';
...
MIT © [ezrahel](https://github.com/ezrahel)
