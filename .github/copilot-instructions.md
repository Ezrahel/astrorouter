...
- **packages/vite-plugin** - Vite integration (`@astroroute/vite`)
- **packages/next-plugin** - Next.js integration (`@astroroute/next`)
...
- `BASE_DOMAIN` - Tunnel domain (e.g., `astroroute.dev`)
...
- Checks both `*.astroroute.app` subdomains and custom domains in the `domains` table
...
- Publishes `astroroute` CLI to npm
